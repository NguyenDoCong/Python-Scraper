...
class TiktokLogin(Scraper):
    ...
    def __init__(self) -> None:
        super().__init__()
        self._base_url = "https://www.tiktok.com/login"
        self._driver = webdriver.Chrome(options=self._chrome_driver_configuration())
        self._driver.get(self._base_url)
        self._cookie_term_css_selector = "._42ft._4jy0._al65._4jy3._4jy1.selected._51sy"
        self._input_text_css_selector = "//input[@type='text']"
    ...
    def _load_tiktok_cookies(self) -> None:
        try:
            self._driver.delete_all_cookies()
            with open(Config.TIKTOK_FILE_PATH, "rb") as file:
                cookies = pickle.load(file)
                for cookie in cookies:
                    print(cookie)
                    try:
    ...
    def _load_tiktok_cookies_and_refresh_driver(self) -> None:
        """Load cookies and refresh driver"""
        self._load_tiktok_cookies()
        self._driver.refresh()
    ...
    def _close_cookie_term(self) -> None:
        ...
        try:
            ...
            logs.log_error(f"An error occurred while closing cookie term {e}")
    def _save_tiktok_cookies(self) -> None:
        """
        Save cookies with log in account to json file
        """
        try:
            cookies = self._driver.get_cookies()
            with open(Config.TIKTOK_FILE_PATH, "wb") as file:
                pickle.dump(cookies, file)
        except Exception as e:
            logs.log_error(f"An Error occurred while saving cookies: {e}")
    def _save_tiktok_browser(self) -> None:
        """
        Click button to save browser
        """
        try:
            self._wait.until(
                EC.presence_of_element_located(
                    (By.XPATH, "//div[@aria-label='Tiktok']")
                )
            )
            continue_button = self._driver.find_element(
                By.XPATH, self._submit_button_selector
    ...
    def _tiktok_login(self) -> None:
        ...
        try:
            ...
            # user_name.send_keys(Config.TIKTOK_EMAIL)
            # password.send_keys(Config.TIKTOK_PASSWORD)
            ...
            # log_in_button.click()
            time.sleep(5)
            fb_login_btn = self.driver.find_elements_by_xpath("//*[contains(text(), 'Facebook')]")
            fb_login_btn.send_keys(Keys.ENTER)
            time.sleep(100)
    ...
    def login_tiktok_pipeline(self) -> None:
        """
        Pipeline to log in on an account without 2-step verification
        """
        try:
            ...
            rprint("Step 2 of 4 - Logging in")
            self._tiktok_login()
            # self._load_tiktok_cookies_and_refresh_driver()
            rprint("Step 3 of 4 - Saving browser")
            self._save_tiktok_browser()
            rprint("Step 4 of 4 - Saving cookies")
            self._save_tiktok_cookies()
